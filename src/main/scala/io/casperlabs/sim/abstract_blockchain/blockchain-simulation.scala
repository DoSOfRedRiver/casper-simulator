...
import io.casperlabs.sim.blockchain_components.execution_engine.{NodeId, Transaction, ValidatorId}
...
trait BlockchainSimulationFactory[MS] {
  ...
  def createNewSimulation(
                           ...
                           transactionsStream: DeploysStream,
...
/**
  * Abstraction of the deploys generator.
...
trait DeploysStream {
  ...
  /**
    * Creates next scheduled deploy.
    * It is required that a given instance of DeployStream produces items with monotonic delivery timepoints.
    */
  def generateNext(): ScheduledDeploy
}
...
/**
  ...
  * against a diversity of blockchain architectures we are going to simulate.
  *
  * @tparam MS memory states type
  */
trait BlockchainObserver[MS] {

  def blockProposed(time: Timepoint, signalledBy: ValidatorId, block: AbstractBlock): Unit

  def blockDelivered(time: Timepoint, signalledBy: ValidatorId, block: AbstractBlock): Unit

  def blockFinalized(time: Timepoint, signalledBy: ValidatorId, block: AbstractBlock, postState: MS): Unit
